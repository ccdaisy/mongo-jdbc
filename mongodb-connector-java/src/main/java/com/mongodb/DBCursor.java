...
public class DBCursor implements Iterator<DBObject>, Iterable<DBObject> {
	 ...
	 */
	public DBCursor(Mongo m, String namespace, DBObject q, DBObject k) {
	...
	public DBCursor copy() {
		DBCursor c = new DBCursor(_m, _namespace, _query, _keysWanted);
		c._orderBy = _orderBy;
		c._hint = _hint;
	...
	/**
	 * Returns an object containing basic information about the exectution of
	 * the query that created this cursor This creates a <code>DBObject</code>
	 * with the key/value pairs: "cursor" : cursor type "nScanned" : number of
	 ...
	 * </p>
	 * <p>
	 * <b>Note</b>: Specifying a <i>negative number</i> instructs the server to
	 * return that number of items and to close the cursor. It will only return
	 * what can fit in a <i>single 4mb response</i>.
	 * </p>
	 * 
	 * @param n
	...
	private void _check() throws MongoException {
		...
		if (_namespace != null && _query != null) {
			...
			_it = _m._find(_namespace, foo, _keysWanted, _skip, _numWanted, _options);
		}
	...
	void _addToQueryObject(DBObject query, String field, DBObject thing,
			boolean sendEmpty) {
		if (thing == null)
			return;
	...
	/**
	 * Converts this cursor to an array. If there are more than a given number
	 * of elements in the resulting array, only return the first <tt>min</tt>.
	 * 
	 * @param min
